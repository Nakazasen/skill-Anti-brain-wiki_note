...
ABW OS không phải một bộ prompt. Nó là lớp kỷ luật vận hành cho agent:

1. **Nhớ trước khi trả lời**: đọc `.brain/` và `wiki/` trước khi kết luận.
2. **Truy vết trước khi tự tin**: fact quan trọng phải trỏ được về nguồn hoặc được ghi vào gap log.
3. **Grounding trước khi import**: NotebookLM là grounding engine, không phải chỗ đổ file vô kiểm soát.
4. **Đóng gói trước khi mở rộng**: khi gần chạm giới hạn source của NotebookLM, dùng `/abw-pack` để tạo package nén có traceability.
5. **Duyệt trước khi sync**: `/abw-sync` mặc định dry-run và chỉ upload khi operator xác nhận rõ.
6. **Nghiệm thu trước khi chấp nhận**: dùng `/abw-eval`, `/abw-audit`, và `/abw-lint` để tránh nhận bừa đầu ra nông.
